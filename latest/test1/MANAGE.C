	   ...
	   if((ch=fgetc(fp))!=EOF)//!feof(fp)
        {
            ...
            Put_String(23,94+i*17,2,2,0,s[i]);
            i++;
        }
       else
       {
	   ...
	   if(ch=fgetc(fp)!=EOF)
		{
			...
			fgets(s[i],30,fp);
			fseek(fp,2L,SEEK_CUR);//转义字符\n
			Put_String(30,115+i*17,2,2,0,s[i]);
			i++;
		}
	   else
	   {
...
void delete_query_in_all(Rec node)
{
	...
	delete_node(list,pos);  //在all的list中删除这个结点
    rewrite_file(list,ALL);   
}
...
void current_state(Pp *p)
{
    ...
    fill_Rectangle(300,537,320,168,1);
    fill_Rectangle(50,174,70,182,1);
    fill_Rectangle(162,160,180,168,1);
    fill_Rectangle(162,174,180,182,1);

	Put_String(52,161,2,2,64,itoa(count_port(p)[0],ch,10));  //显示各类车位数量
	Put_String(52,175,2,2,184,itoa(count_port(p)[1],ch,10));
	Put_String(164,161,2,2,11,itoa(count_port(p)[2],ch,10));
	Put_String(164,175,2,2,28,itoa(count_port(p)[3],ch,10));
}
            ...
            case BACKSPACE: if(i>0)       //退格
                            {
                                s[--i]='\0';
                                fill_Rectangle(button->x1,button->y1,button->x2,button->y2,0x5FBF);
								Put_String(button->x1+5,button->y1+5,1,1,0,s); 
                                continue;
                            }
            default: s[i]=k;//任何一个字符都可以用相应的ASCII表示
                     s[i+1]='\0';
